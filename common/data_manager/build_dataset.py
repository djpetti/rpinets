...
def _process_label(label, path, disk_cache):
  ...
  for image in os.listdir(label_path):
    ...
    image_id = "%s_%s" % (label, image)
    image_names.append((image_id, None))

  return image_names

def _build_dataset(path, disk_cache):
  ...
  for label in os.listdir(path):
    ...
    dataset_images.extend(_process_label(label, path, disk_cache))

  # Create and save a dataset. (The batch size argument doesn't really matter
  # here.)
  data = dataset.Dataset(dataset_images, disk_cache, 0)
  return data
...
def convert_dataset(location):
  ...
  train_set = _build_dataset(train_path, disk_cache)
  test_set = _build_dataset(test_path, disk_cache)

  # Save the datasets to the disk.
  print "Saving dataset_train.pkl..."
  train_set.save_images("dataset_train.pkl")
  print "Saving dataset_test.pkl..."
  test_set.save_images("dataset_test.pkl")
