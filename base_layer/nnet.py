...
def dropout(layer_output, keep_prob, is_training, noise_shape=None, seed=None,
  ...
  if sg.backend_name == "theano":
    ...
    distribution = sg.random.binomial(size=noise_shape, p=keep_prob)
    dropped_out = sg.backend.tensor.switch(distribution, layer_output, 0)

  elif sg.backend_name == "tensorflow":
    dropped_out = sg.backend.nn.dropout(layer_output, keep_prob,
...
def relu(features, name=None):
  ...
  if sg.backend_name == "theano":
    return sg.backend.tensor.nnet.relu(features)
  elif sg.backend_name == "tensorflow":
    return sg.backend.nn.relu(features, name=name)
...
def softmax(logits, name=None):
  ...
  if sg.backend_name == "theano":
    return sg.backend.tensor.nnet.softmax(logits)
  elif sg.backend_name == "tensorflow":
    return sg.backend.nn.softmax(logits, name=name)
...
def softmax_cross_entropy(logits, labels, name=None):
  ...
  if sg.backend_name == "theano":
    ...
    return sg.backend.tensor.nnet.categorical_crossentropy(scaled, labels)

  elif sg.backend_name == "tensorflow":
    return sg.backend.nn.softmax_cross_entropy_with_logits(logits, labels,
                                                           name=name)
...
def conv2d(inputs, filters, strides, border_mode):
  ...
  Returns:
    A convolution op. """
  if sg.backend_name == "theano":
    return sg.backend.tensor.nnet.conv2d(inputs, filters, subsample=strides,
    ...
    return sg.backend.nn.conv2d(inputs, filters, strides, border_mode,
                                data_format="NCHW")
...
def max_pool(inputs, kernel_shape, strides, padding):
  ...
  if sg.backend_name == "theano":
    ...
    return sg.backend.tensor.signal.pool.pool_2d(inputs, kernel_shape,
                                                 ignore_border=ignore_border,
                                                 st=strides)
  elif sg.backend_name == "tensorflow":
    return sg.backend.nn.max_pool(inputs, kernel_shape, strides, padding)
...
def local_response_normalization(data, depth_radius, bias, alpha, beta):
  ...
  if sg.backend_name == "theano":
    ...
    extra_channels = sg.backend.tensor.alloc(0, batch, maps + 2 * half, x, y)
    square = sg.backend.tensor.set_subtensor( \
    ...
    return sg.backend.nn.local_response_normalization(data, depth_radius,
